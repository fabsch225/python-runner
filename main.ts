  ...
  async onload() {
    ...
    this.registerMarkdownCodeBlockProcessor("xpython", async (source, el, ctx) => {
      ...
      button.onclick = async () => {
        ...
        const outputEl = el.createDiv({ cls: "python-output terminal-output" });
        ...
        // Wrap user code to save matplotlib plot if used
        ...
        if (/import\s+matplotlib|from\s+matplotlib|import\s+pyplot|from\s+matplotlib\.pyplot/i.test(source)) {
          ...
          // If user code does not already save a figure, append savefig and close
          ...
          (error, stdout, stderr) => {
            ...
            if (fs.existsSync(plotFile)) {
              try {
                ...
                // Invert if dark mode
                const isDark = document.body.classList.contains('theme-dark') || document.body.classList.contains('dark');
                imgTag = `<img src="data:image/png;base64,${base64}" style="max-width:100%;border-radius:6px;margin-top:10px;${isDark ? 'filter:invert(1) hue-rotate(180deg);' : ''}"/>`;
              } catch (e) {
    ...
    this.registerMarkdownCodeBlockProcessor("xr", async (source, el, ctx) => {
      ...
      button.onclick = async () => {
        ...
        const outputEl = el.createDiv({ cls: "r-output terminal-output" });
        outputEl.style.background = "#222";
        outputEl.style.color = "#b5e853";
        outputEl.style.fontFamily = "monospace";
        outputEl.style.padding = "12px";
        ...
        let plotCode = '';
        let plotCount = 0;
        // We'll wrap each plot() call with a new png device
        // Replace plot( with {dev.off(); png(...); plot(
        // But only if user does not open a device themselves
        const deviceRegex = /(^|\s)(png|svg|jpeg|tiff|bmp|pdf)\s*\(/i;
        let wrappedR = '';
        if (deviceRegex.test(source)) {
          // User opens a device, just run their code
          wrappedR = `tryCatch({\n${source}\n}, error=function(e) print(e))`;
        } else {
          // Find all plot( calls and wrap them
          let code = source;
          // Find all plot( calls
          ...
          while ((match = plotCallRegex.exec(code)) !== null) {
            const before = code.slice(lastIndex, match.index + match[0].length - 5); // up to 'plot'
            newCode += before;
            // Insert device open
            ...
            for (const plotFile of plotFiles) {
              if (fs.existsSync(plotFile)) {
                try {
                  ...
                  // Invert if dark mode
                  const isDark = document.body.classList.contains('theme-dark') || document.body.classList.contains('dark');
                  imgTags += `<img src="data:image/png;base64,${base64}" style="max-width:100%;border-radius:6px;margin-top:10px;${isDark ? 'filter:invert(1) hue-rotate(180deg);' : ''}"/>`;
                } catch (e) {
